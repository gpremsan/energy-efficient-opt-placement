...
def calculate_demand_vector_varying_busyness(df, num_time_slots, num_apps, num_BSs, time_slot_in_minutes):
    ...
    for row_idx in range(num_apps):
        ...
        # For each time slot, decide which are the busiest
        for t in range(len(ts_resampled)):
            ...
            # print(busy_BSs, moderate_BSs, less_busy_BSs)
            ...
            # demand_vector[:, row_idx, :] = ts_per10min.values.tolist()
    ...
    # Plot demand vector over 144 time slots
...
def plot_demand_per_app_group(demand_vector, app_groups):
    plt.rcParams.update({'font.family': 'sans-serif'})
    plt.rcParams.update({'font.sans-serif': 'Helvetica'})
    plt.rcParams.update({'font.size': 14})
    demand_vector = np.array(demand_vector)
    for group_name, app_indices in app_groups.items():
        demand_per_slot = np.sum(demand_vector[:, app_indices, :], axis=(0, 1))
        fig, ax = plt.subplots()
        plt.plot([*range(0, len(demand_per_slot))], demand_per_slot)
        ...
        plt.tight_layout()
        # plt.show()
        plt.savefig('plot-azure-demand-{}-7days.png'.format(group_name), dpi=600)
        #plt.cla()
        #plt.close(fig)
    return
...
np.random.seed(10)
azure_dataset_path = ""
if len(azure_dataset_path) == 0:
    print("Update the path to the dataset before running script")
    exit()
...
for day in range(1, 8):
    function_trace_df = read_azure_trace_by_hash_function(day, hash_functions,
                                                               azure_dataset_path.rstrip("/") + "/")
    demand_vector = calculate_demand_vector_varying_busyness(function_trace_df, num_time_slots, num_apps,
                                                                     num_BSs, time_slot_in_minutes)
    all_demand_vectors.append(demand_vector.copy())

overall_demand = np.concatenate(tuple(all_demand_vectors[day-1] for day in range(1,8)), axis=2)
print(overall_demand.shape)
plot_demand_per_app_group(overall_demand, app_groups.copy())
